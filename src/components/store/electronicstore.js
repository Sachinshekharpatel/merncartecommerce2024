...
const ElectronicStore = () => {
  ...
  useEffect(() => {
    ...
    .then((data) => {
      const electronicProducts = data.filter((product) => {
        return product.category === "electronics";
      });

    setDataToDisplay(electronicProducts);
    })
    .catch((error) => {
  ...
  return (
    ...
    <div
      ...
      >
      <button onClick={() => navigate("/")} style={{ color: "grey", marginBottom: "140px" }}>
          Home |{" "}
        </button>
        <button onClick={() => navigate("/womenstore")} style={{ color: "grey", marginBottom: "140px" }}>
          | Women |
        </button>
        <button onClick={() => navigate("/menstore")} style={{ color: "grey", marginBottom: "140px" }}>
          | Men
        </button>
    </div>
